...
| Corps reservoir storage observations | U.S. Army Corps of Engineers CWMS Data API, Pacific Northwest Region office `NWDP`: `https://cwms-data.usace.army.mil/cwms-data/timeseries`, `unit=ac-ft`, keyless | Daily storage records (each day's last reading) for the admitted Columbia Basin locations in Idaho, Oregon, Washington and Montana | `refresh_reservoirs.py` validates and commits them into the same normalized `reservoirs.json`. The office and the whole six-part series name are pinned per location in `admitted_cwms_reservoirs.json`; `tools/audit_cwms_stations.py` gathers admission evidence, and every location kept out is named there with its finding | An individual failure keeps the last known record; a broadly failed run does not replace the complete payload. A response for another office, series or unit is refused rather than read | No geometry is taken from this service. Forecast (`FCST`) and republished Reclamation (`USBR`) versions are refused by the loader. The provider publishes no full level in acre-feet, so every denominator comes from the dam inventory under ADR-003/ADR-072. Admitted under ADR-102 on 2026-08-29. |
| CAP reservoir storage observation | Central Arizona Project Lake Pleasant operations endpoint, the service behind the public graphic: `https://azr-prod-rsg-dmz-app-waterqualityweb.azurewebsites.net/api/opslakepleasant`, keyless, one current record | Today's storage for Lake Pleasant, merged into the dense-history cache so a series accumulates | `refresh_reservoirs.py` reads the one record, refuses it if any pinned field is missing, and merges it into `data/reservoir-source-history.json`; `admitted_cap_reservoirs.json` pins the endpoint and records the full-level finding | An individual failure keeps the last known record and the ordinary freshness path withdraws it after the window | No geometry is taken from this service. No history route exists; no terms are published. Denominator from the dam inventory under ADR-003; admitted under ADR-104 on 2026-08-29. |
